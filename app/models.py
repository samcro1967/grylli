...
class Reminder(db.Model):
    ...
    recurrence_rule = db.Column(db.String(256), nullable=True)  # e.g. "FREQ=WEEKLY;BYDAY=MO"
    ...
    # Destinations (many-to-many, just like in Message)
    apprise_destinations = db.relationship(
    ...
    email_messages = db.relationship(
        "EmailMessage",
        secondary="reminder_email_links",
        backref="linked_reminders",
    )

    def __repr__(self):
        """
        Return a string representation of the Reminder instance,
        including label, start date, and recurrence rule.
        """
...
class SecurityQuestions(db.Model):
    ...
    set_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
