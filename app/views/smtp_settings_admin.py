...
def view_smtp_config():
    ...
    try:
        ...
        for setting in settings:
            if setting.is_sensitive:
                try:
                    ...
                    log_exception_with_traceback(
                        f"Admin user '{current_user.username}' failed to decrypt setting '{setting.key}'",
                        e,
                    )
        ...
        log_exception_with_traceback(
            f"Admin user '{current_user.username}' encountered error loading SMTP settings", e
        )
...
def send_test_email():
    ...
    try:
        ...
        log_info_message(
            f"Admin user '{current_user.username}' sent a test email using system SMTP settings."
        )
        flash(_("Test email sent successfully."), "success")
    except Exception as e:
        log_exception_with_traceback(
            f"Admin user '{current_user.username}' failed to send test SMTP email", e
        )
