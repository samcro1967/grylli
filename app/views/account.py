...
def set_security_questions():
    # Allow users to update their questions if already set
    ...
    if existing and form.validate_on_submit():
        ...
        db.session.commit()
        flash(_("Security questions updated successfully."), "success")
        return redirect(url_for("index.index"))

    elif not existing and form.validate_on_submit():
        save_security_questions(current_user, form)
        flash(_("Security questions saved."), "success")
        return redirect(url_for("index.index"))

    return render_template("account/set_security_questions.html", form=form)
