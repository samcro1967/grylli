...
def create_reminder():
    ...
    Handles the creation of new reminders. This version auto-assigns a temporary
    start_at value to satisfy DB constraints, since scheduling is handled separately.
    """
    form = ReminderForm()
    try:
        if form.validate_on_submit():
            ...
            reminder = Reminder(
                ...
                start_at=datetime(2000, 1, 1),  # ✅ Add a temporary default
                is_enabled=True,
                created_at=now,
                updated_at=now,
            )
...
def schedule_reminder(reminder_id):
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            log_info_message(
                f"User '{current_user.username}' submitted an invalid schedule form for reminder ID={reminder.id}."
            )
