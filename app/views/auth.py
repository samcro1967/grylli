    ...
    if form.validate_on_submit():
        try:
            ...
            if user and user.locked_until and user.locked_until > datetime.utcnow():
                ...
                log_error_message(f"User '{username}' attempted login during active lockout.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password_hash, form.password.data):
            ...
            next_page = unquote(request.args.get("next", ""))
            next_page = unquote(request.args.get("next", ""))
            return redirect(get_safe_redirect(next_page, fallback_endpoint="index.index"))
        ...
        if user and attempts >= 10:
            ...
            log_error_message(
                f"User '{username}' locked out after {attempts} failed login attempts."
            )
            return render_template("auth/login.html", form=form)
...
def forgot_password():
    ...
    if delay > 0:
        log_info_message(
            f"Redirecting to rate-limit page for user '{user.username}' due to failed answers with delay={delay}s"
        )
    ...
    if session["reset_attempts"] > 3:
        ...
        log_error_message(f"Too many failed security question attempts for {user.username}")
        return redirect(url_for("auth.forgot_password"))
    ...
    try:
        if user.verify_security_answers(answers):
            ...
            log_info_message(f"Password reset email sent to '{user.email}'.")
            ...
            log_info_message(f"Incorrect security answers for user '{user.username}'")
            return render_template(
                "auth/forgot_password.html",
                form=form,
                security_questions=user.get_security_questions(),
...
def activate_account(token):
    ...
    try:
        ...
        log_exception_with_traceback("Activation token expired or invalid", e)
...
# ---------------------------------------------------------------------
@bp.route("/verify_security_question", methods=["GET", "POST"])
