...
def index():
    try:
        ...
        log_info_message(f"Access - {current_user.username} - User SMTP")
...
# ---------------------------------------------------------------------
@bp.route("smtp/create", methods=["GET", "POST"], strict_slashes=False)
@login_required
def create():
    form = SmtpForm()
    try:
        ...
        if form.errors:
            log_info_message(
                f"User '{current_user.username}' submitted invalid SMTP creation form: {form.errors}"
            )
...
# ---------------------------------------------------------------------
@bp.route("smtp/<int:smtp_id>/edit", methods=["GET", "POST"], strict_slashes=False)
@login_required
def edit(smtp_id):
    try:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            if form.smtp_password.data and form.smtp_password.data != "********":
                smtp.smtp_password = form.smtp_password.data
                log_info_message(
                    f"User '{current_user.username}' updated SMTP password for config ID={smtp.id}"
                )
        ...
        if form.errors:
            log_info_message(
                f"User '{current_user.username}' submitted invalid SMTP edit form: {form.errors}"
            )
...
# Delete SMTP config
# ---------------------------------------------------------------------
@bp.route("smtp/<int:smtp_id>/delete", methods=["POST"], strict_slashes=False)
@login_required
def delete(smtp_id):
    try:
        smtp = UserMailSettings.query.filter_by(id=smtp_id, user_id=current_user.id).first_or_404()
...
# Send Test Email
# ---------------------------------------------------------------------
@bp.route("smtp/<int:smtp_id>/send-test", methods=["POST"], strict_slashes=False)
@login_required
def send_test(smtp_id):
    try:
        smtp = UserMailSettings.query.filter_by(id=smtp_id, user_id=current_user.id).first_or_404()
        success, message = send_test_smtp(current_user, smtp)
        ...
        if success:
            ...
            log_info_message(
                f"User '{current_user.username}' failed test email using SMTP config ID={smtp.id}. Reason: {message}"
            )
            flash(_("Failed to send test email. ") + str(message), "danger")
    except Exception as e:
