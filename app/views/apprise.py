...
def config():
    ...
    """
    try:
        log_info_message(f"User '{current_user.username}' accessed Apprise configuration.")
        apprise_urls = (
...
def edit_apprise(id):
    ...
    """
    # Retrieve the Apprise destination
    ...
    template = (
        "apprise/create_apprise_partial.html"  # Partial view for HTMX requests
        if request.headers.get("HX-Request")
        else "apprise/edit_apprise.html"  # Full view for regular requests
    )

    return render_template(template, form=form, apprise=apprise)
