...
def mfa_settings():
        ...
        if form.enabled.data:
            if not initial_enabled or not current_user.mfa_enabled:
                if not form.code.data:
                    ...
                    log_info_message(
                        f"User '{current_user.username}' attempted to enable MFA without providing code."
                    )
                else:
                    ...
                    log_info_message(
                        f"User '{current_user.username}' provided invalid TOTP code during MFA setup."
                    )
        else:
    ...
    if not current_user.mfa_secret:
        ...
        db.session.commit()
        log_info_message(f"User '{current_user.username}' was assigned a new MFA secret.")
    ...
    if request.method == "GET":
        ...
        log_info_message(f"User '{current_user.username}' loaded MFA settings page.")
...
# Show recovery codes
# ---------------------------------------------------------------------
@bp.route("/recovery-codes", methods=["GET", "POST"])
@login_required
def show_recovery_codes():
    ...
    try:
        ...
        log_info_message(f"User '{current_user.username}' accessed MFA recovery codes page.")
    except Exception as exc:
        flash(_("Could not load recovery codes."), "danger")
        log_info_message(
            f"User '{current_user.username}' failed to decrypt MFA recovery codes: {str(exc)}"
        )
        return redirect(url_for("index.index"))
    ...
    return render_template("mfa/recovery_codes.html", recovery_codes=codes)
    ...
    if form.validate_on_submit():
        ...
        if user.mfa_recovery_codes:
            try:
                ...
                codes = []
                log_info_message(f"User '{user.username}' had invalid recovery code data.")
            if code in codes:
                codes.remove(code)
                if not codes:
