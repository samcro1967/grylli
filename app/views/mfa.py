...
def mfa_settings():
            ...
            if form.enabled.data:
                if not initial_enabled or not current_user.mfa_enabled:
                    if not form.code.data:
                        ...
                        log_info_message(f"User '{current_user.username}' tried to enable MFA without a code.")
                    else:
                        ...
                        log_info_message(f"User '{current_user.username}' provided invalid TOTP code.")
            else:
    ...
    template = "mfa/settings_partial.html" if request.headers.get("HX-Request") == "true" else "mfa/settings.html"
...
def show_recovery_codes():
    ...
    try:
        ...
        log_exception_with_traceback("Failed to decrypt MFA recovery codes", exc)
    ...
    from flask_wtf import FlaskForm  # ✅ Add this import at the top of the file
