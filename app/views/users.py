...
def edit_user(user_id):
    ...
    if not user:
        ...
        log_info_message(f"Admin '{current_user.username}' attempted to edit missing user ID={user_id}.")
        return redirect(url_for("users.list_users"))
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            log_info_message(f"Admin '{current_user.username}' updated user '{user.username}' (ID={user.id}).")
            return redirect(url_for("users.list_users"))
        except Exception:
            db.session.rollback()
            flash(_("An error occurred while updating the user."), "danger")
            log_exception_with_traceback("Failed to update user.")
