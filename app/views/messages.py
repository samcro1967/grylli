...
def edit_message(message_id):
    ...
    try:
        ...
        # Determine which template to render (full or partial)
        template = (
            "messages/create_message_partial.html"  # Partial view for HTMX requests
            if request.headers.get("HX-Request")
            else "messages/edit_message.html"  # Full view for regular requests
        )
...
def assign_destinations(message_id):
    ...
    try:
        ...
        # Fetch available Apprise URLs and Webhooks for the current user
        apprise_options = (
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            # Assign selected destinations to the message
        ...
        # Determine which template to render based on HTMX request
        template = (
            "messages/assign_destinations_partial.html"  # Partial view for HTMX requests
            if request.headers.get("HX-Request")
            else "messages/assign_destinations_full.html"  # Full view for regular requests
        )
...
def schedule_message(message_id):
    ...
    try:
        ...
        template = (
            "messages/schedule_message_partial.html"  # Partial view for HTMX requests
            if request.headers.get("HX-Request")
            else "messages/schedule_message_full.html"  # Full view for regular requests
        )
...
def view_message(message_id):
    ...
    try:
        ...
        # Determine if this is an HTMX request or a regular request
        template = (
            "messages/view_message_partial.html"  # Use partial view for HTMX requests
            if request.headers.get("HX-Request")
            else "messages/view_message_full.html"  # Use full view for regular requests
        )
...
def send_test_message(message_id):
    ...
    if success:
        log_info_message(
            f"User '{current_user.username}' sent test message ID={message_id}. "
            f"Apprise: {len(message.apprise_destinations)}, Webhooks: {len(message.webhooks)}"
        )
...
def _send_test_via_apprise_and_webhooks(message, username):
    ...
    Args:
        ...
        username (str): The username of the user sending the test.
