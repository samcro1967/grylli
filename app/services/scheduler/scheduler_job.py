...
def start_scheduler(app):
    ...
    def task_wrapper():
        with app.app_context():
            try:
                log_info_message(
                    "🌀 APScheduler processing check-in reminders and overdue actions..."
                )
                process_checkins_and_overdue_actions()
            except Exception:
                log_info_message("❌ Error in check-in/overdue task:\n" + traceback.format_exc())
    ...
    def backup_wrapper():
        with app.app_context():
            try:
                ...
                log_info_message("❌ Error in daily backup task:\n" + traceback.format_exc())
    ...
    def reminder_wrapper():
        with app.app_context():
            try:
                ...
                log_info_message("❌ Error in reminder task:\n" + traceback.format_exc())

    # Wrapper for version check job
    def version_check_wrapper():
        with app.app_context():
            try:
                log_info_message("🔍 Checking for newer Grylli releases...")
                check_latest_version()
            except Exception:
                log_info_message("❌ Error in version check task:\n" + traceback.format_exc())
