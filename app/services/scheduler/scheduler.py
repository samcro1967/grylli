...
def process_checkins_and_overdue_actions():
    ...
    try:
        ...
        log_error_message(
            f"❌ Error in check-in reminder processing: {e}\n{traceback.format_exc()}"
        )

    try:
        find_expired_items()
    except Exception as e:
        log_error_message(f"❌ Error in expired item processing: {e}\n{traceback.format_exc()}")
            ...
            if item.reminder_sent_at is None:
                try:
                    ...
                    if user:
                        ...
                        log_error_message(
                            f"❌ Cannot send reminder — user {item.user_id} not found"
                        )
                except Exception as e:
                    db.session.rollback()
                    log_error_message(
                        f"❌ Error sending reminder for {item.label}: {e}\n{traceback.format_exc()}"
                    )
...
def find_expired_items():
    ...
    for item in messages:
        if (
            ...
            log_info_message(f"⏭️ Skipping {item.label} — not eligible for expiration")
            continue
        ...
        if now - item.last_checkin >= grace:
            try:
                ...
                db.session.rollback()
                log_error_message(
                    f"❌ Error executing message '{item.label}': {e}\n{traceback.format_exc()}"
                )
    ...
    for item in emails:
        ...
        if now - item.last_checkin >= grace:
            try:
                ...
                db.session.rollback()
                log_error_message(
                    f"❌ Error executing email '{item.label}': {e}\n{traceback.format_exc()}"
                )
...
def execute_due_reminders():
    ...
    for reminder in reminders:
        ...
        if not reminder.is_enabled or not reminder.start_at:
            log_info_message(f"⏭️ Skipping {reminder.label} — not enabled or missing start_at")
            continue
        ...
        if reminder.start_at > now:
            log_info_message(
                f"⏭️ Skipping {reminder.label} — start_at in future: {reminder.start_at}"
            )
            continue

        if reminder.max_occurrences and reminder.occurrences_sent >= reminder.max_occurrences:
            log_info_message(
                f"⏭️ Skipping {reminder.label} — max occurrences reached ({reminder.occurrences_sent})"
            )
        ...
        if reminder.recurrence_rule:
            ...
            if reminder.last_sent_at:
                log_info_message(f"⏭️ Skipping {reminder.label} — already sent one-time reminder")
                continue
        ...
        try:
            ...
            if reminder.apprise_destinations:
                ...
                log_info_message(f"📭 No Apprise destinations for {reminder.label}")

            if reminder.webhooks:
                execute_webhooks(reminder)
            else:
                log_info_message(f"📭 No Webhook destinations for {reminder.label}")

            if reminder.smtp_configs:
                send_reminder_email(reminder)
            else:
                log_info_message(f"📭 No SMTP config for {reminder.label} — skipping email")
            ...
            if not reminder.recurrence_rule:
                reminder.is_enabled = False
                log_info_message(f"🔕 One-time reminder disabled: {reminder.label}")

            log_info_message(
                f"📝 Committing reminder: {reminder.label}, enabled={reminder.is_enabled}"
            )
            db.session.commit()
            ...
            db.session.rollback()
            log_error_message(
                f"❌ Error sending reminder '{reminder.label}': {e}\n{traceback.format_exc()}"
            )
