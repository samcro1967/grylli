...
def create_backup():
    ...
    try:
        ...
        log_info_message(f"✅ Backup created at: {dest_file}")
        return dest_file
    except Exception as e:
        log_error_message(f"❌ Failed to create database backup: {e}")
        return None
...
def get_backup_files():
    ...
    try:
        ...
        log_info_message(f"📦 Found {len(sorted_files)} backup file(s).")
        return sorted_files
    except Exception as e:
        log_error_message(f"❌ Failed to list backup files: {e}")
        return []
...
def delete_old_backups(days_to_keep=7):
    ...
    try:
        ...
        if not os.path.exists(dir_path):
            log_info_message("📁 Backup directory does not exist. Skipping deletion.")
            return
        ...
        for f in os.listdir(dir_path):
            ...
            try:
                ...
                if (now - mtime).days > days_to_keep:
                    os.remove(path)
                    log_info_message(f"🗑️ Deleted old backup: {f}")
                    deleted_count += 1
            except Exception as e:
                log_error_message(f"⚠️ Failed to delete backup {f}: {e}")
                continue

        log_info_message(f"🧹 Cleanup complete. Deleted {deleted_count} old backup(s).")

    except Exception as e:
        log_error_message(f"❌ Unexpected error during backup cleanup: {e}")
