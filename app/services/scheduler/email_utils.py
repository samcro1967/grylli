...
def _send_email(
    ...
    try:
        ...
        log_debug_message(
            f"📡 SMTP Host: {smtp_settings.smtp_host}, Port: {smtp_settings.smtp_port}, "
...
def send_checkin_email(user, item):
    ...
    try:
        log_info_message(
            f"✉️ User '{user.username}' — Sending check-in reminder to {user.email} for {item.label}"
        )
        ...
        log_error_message(f"❌ Exception in send_checkin_email: {e}")
        log_error_message(traceback.format_exc())
...
def send_secure_email(item):
    ...
    try:
        ...
        log_info_message(
            f"📤 User '{user.username}' — Sending email: {item.label} → {item.recipient}"
        )
        ...
        log_error_message(
            f"❌ Exception in send_secure_email for {getattr(item, 'label', 'unknown')}: {e}"
        )
        log_error_message(traceback.format_exc())
...
def send_owner_notice(item):
    ...
    try:
        ...
        log_info_message(
            f"📬 User '{user.username}' — Sent final notice for {item.label} to {user.email}"
        )
        _send_email(user.email, subject, body, smtp_settings)

    except Exception as e:
        log_error_message(f"❌ Exception in send_owner_notice: {e}")
        log_error_message(traceback.format_exc())
...
def send_reminder_email(reminder):
    ...
    try:
        ...
        if not user:
            log_error_message(
                f"❌ Reminder email failed — user not found for reminder {reminder.label}"
            )
            return

        if not smtp:
            log_error_message(
                f"❌ Reminder email failed — no SMTP config for user {reminder.user_id}"
            )
            return
        ...
        log_info_message(
            f"📨 User '{user.username}' — Sending reminder email to {user.email} via SMTP config {smtp.id}"
        )
        success = _send_email(user.email, subject, body, smtp)

        if not success:
            log_error_message(f"❌ Reminder email failed to send for {reminder.label}")

    except Exception as e:
        log_error_message(f"❌ Exception in send_reminder_email: {e}")
        log_error_message(traceback.format_exc())
