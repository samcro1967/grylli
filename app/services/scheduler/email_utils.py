...
def send_checkin_email(user, item):
    ...
    if not smtp_settings or not smtp_settings.enabled:
        log_error_message(f"⚠️ SMTP settings not found or disabled for user {getattr(user, 'id', 'unknown')}")
        return
...
def send_reminder_email(reminder):
    ...
    Args:
        reminder: Reminder object (must have .user_id, .subject, .content).
    """
    ...
    if not smtp:
        log_error_message(f"❌ Reminder email failed — no SMTP config for user {reminder.user_id}")
        return

    subject = f"[Grylli] {reminder.subject}"
    body = reminder.content or f"You have a reminder: {reminder.label}"

    log_info_message(f"📨 Sending reminder email to {user.email} via SMTP config {smtp.id}")
